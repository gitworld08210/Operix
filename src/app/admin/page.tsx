 ...
 *
 * The admin token is entered once and sent as the `x-admin-token` header on
 * every write request (matching the fail-closed server gate). It is kept in
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#141414]">
      ...
      {/* Admin token bar */}
      <div className="bg-[#181818] border-b border-[#2a2a2a] px-4 md:px-12 py-3">
        <label className="flex flex-col sm:flex-row sm:items-center gap-2">
          ...
          <input
            ...
            placeholder="x-admin-token (required for uploads)"
            className="w-full sm:max-w-md px-3 py-2 bg-[#333333] border border-transparent focus:border-[#E50914] text-white rounded text-sm outline-none"
          />
        </label>
      </div>
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="flex-1 p-4 md:p-6">
          ...
          {activeTab === "movie" && <UploadMovieView adminToken={adminToken} />}
          {activeTab === "series" && <SeriesView adminToken={adminToken} />}
          {activeTab === "bulk" && <BulkUploadView adminToken={adminToken} />}
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#1f1f1f] p-6 rounded-lg text-gray-300 space-y-3">
        ...
        <p className="text-sm text-gray-500">
          Every upload requires the admin API token entered above.
        </p>
      </div>
    </div>
  );
...
function UploadMovieView({ adminToken }: { adminToken: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!adminToken) {
      setStatus("error");
      setMessage("Enter the admin API token first.");
      return;
    }
    if (!videoFile) {
      setStatus("error");
...
function SeriesView({ adminToken }: { adminToken: string }) {
  ...
  const handleCreateSeries = async (e: React.FormEvent) => {
    ...
    if (!adminToken) {
      ...
      setCreateMessage("Enter the admin API token first.");
      return;
    }
  ...
  const handleUploadEpisode = async (e: React.FormEvent) => {
    ...
    if (!adminToken) {
      setEpStatus("error");
      setEpMessage("Enter the admin API token first.");
      return;
    }
    if (!seriesId) {
      setEpStatus("error");
...
function BulkUploadView({ adminToken }: { adminToken: string }) {
  ...
  const handleMultiVideoUpload = async () => {
    ...
    for (let i = 0; i < videoFiles.length; i += 1) {
      ...
      const res = await uploadWithProgress("/api/upload/movie", fd, adminToken, (p) => {
